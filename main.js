...
var _parseContent = (baseContent) => {
  ...
  return {
    frontmatter: (_a = match.groups.frontmatter) != null ? _a : "",
    before: (_b = match.groups.before) != null ? _b : "",
    after: (_c = match.groups.after) != null ? _c : ""
  };
};
var _yieldMarkdownForTagDetails = (details) => details.reduce((acc, { fileLink, stringContainingTag }) => {
  ...
  if (existing) {
    ...
    acc.push({ fileLink, stringsContainingTag: [stringContainingTag] });
  }
  return acc;
}, []).map(
  ({ stringsContainingTag, fileLink }) => `> [!quote]+ In ${fileLink}
${stringsContainingTag.map((str) => `> ${str}`).join("\n")}
`
);
var generateTagPageContent = async (app, settings, tagsInfo, tagOfInterest, baseContent = "") => {
  ...
  const filesWithFrontmatterTag = app.vault.getMarkdownFiles().filter((file) => {
    ...
    return (metaMatter == null ? void 0 : metaMatter.tags) ? (metaMatter == null ? void 0 : metaMatter[settings.frontmatterQueryProperty]) !== tagOfInterest && matchesTagOfInterest(metaMatter.tags, tagOfInterest) : false;
...
var extractFrontMatterTagValue = (app, view, frontMatterTag) => {
  ...
  if (view.file) {
    try {
      const metaMatter = (_a = app.metadataCache.getFileCache(
        view.file
...
var TagPagePlugin = class extends import_obsidian2.Plugin {
  ...
  async refreshTagPageContent() {
    ...
    this.app.fileManager.processFrontMatter(activeLeaf.file, (frontMatter) => {
      ...
      (_a = frontMatter.tags) != null ? _a : frontMatter.tags = [];
      frontMatter.tags = [...new Set(frontMatter.tags).add("tag-page-md").add(
        tagOfInterest.slice(1)
        /* Omit the leading # */
      )];
    });
